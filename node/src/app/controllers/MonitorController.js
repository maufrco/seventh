const {host,monitor} = require("../models");
...
class MonitorController {
    ...
    async countMonitorHost(req, res, next){    
        const page = 0;
        try {
            const operation = await monitor.findAll({ 
                group: ['hostId'],
    ...
    async getMonitortById(req, res, next){
        const { id } = req.params;         
        try {
            const operation = await monitor.findAll({where:{hostId: id}, order:[[ 'monitorDate', 'DESC']]})
            if(!operation){
    ...
    async addMonitor(req, res, next){
        try {
            ...
            const operation = await monitor.create({Name:name, Protocol:protocol, Domain:domain, Path: path})
            if(!operation){
